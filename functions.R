trendsmg <- function(inputdf, min, max) {
    ...
    for (i in 1:length(country.l)) {
        tmp <- subset(inputdf, countrycode == country.l[i] & year >= min & year <= max)
        if (dim(tmp)[1] < 10) {
            next
        } else {
...
trendsmg1<- function(inputdf, start1,end1,start2,end2) {
    ...
    country.l<-country.l[country.l!=""]
    for (i in 1:length(country.l)) {
        tmp <- subset(inputdf, countrycode == country.l[i] & year >= start1 & year <= end1)
        if (dim(tmp)[1]==0) {
            next
        } else {
        tmp1<- subset(inputdf, countrycode == country.l[i] & year >= start2 & year <= end2)
        }
        if (dim(tmp1)[1]==0) {
         next
        } else {
